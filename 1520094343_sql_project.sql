...
Answer: Name of facilities that charge fee to members are:
Tennis Court 1
Tennis Court 2
Massage Room 1
Massage Room 2
...
the member name. */
SELECT DISTINCT sub.court, CONCAT( sub.firstname, ' ', sub.surname ) AS name
...
SELECT Facilities.name AS facilityname, CONCAT( Members.firstname,  ' ', Members.surname ) AS name, 
...
SELECT * 
FROM (SELECT Facilities.name AS facilityname, CONCAT( Members.firstname,  ' ', Members.surname ) AS name, 
CASE WHEN Bookings.memid =0
THEN Facilities.guestcost * Bookings.slots
ELSE Facilities.membercost * Bookings.slots
END AS cost
...
SELECT * 
FROM (
SELECT sub1.facilityname, SUM( sub1.cost ) AS totalrevenue
FROM (
